package com.yuntian.autodeliver.controller;
...
public class AutodeliverController {
    ...
    public Integer findResumeOpenState(@PathVariable Long userId) {
        ...
        String url = "http://yuntian-service-resume/resume/openstate/" + userId;  // 指定服务名
        Integer forObject = restTemplate.getForObject(url, Integer.class);
        return forObject;
    }
    ...
    @HystrixCommand(// 线程池标识，要保持唯⼀，不唯⼀的话就共⽤了
     ...
     *
     */
    @GetMapping("/findResumeOpenStateTimeoutFallback2/{userId}")
    @HystrixCommand(
            // 线程池标识，要保持唯一，不唯一的话就共用了
            threadPoolKey = "findResumeOpenStateTimeoutFallback",
            // 线程池细节属性配置
